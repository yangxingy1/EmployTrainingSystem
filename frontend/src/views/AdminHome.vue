  ...
  <div class="admin-shell">
    <aside class="sidebar">
      <div class="brand-area">
        ...
        <div><strong>慧动�?/strong><span>管理中心</span></div>
      </div>
      <nav class="side-menu">
    ...
    <main class="content-area">
      <header class="top-bar">
        ...
        <div class="user-area">
          <div><span>管理�?/span><strong>{{ username }}</strong></div>
      ...
      <section class="stat-grid">
        <div class="stat-card"><span>学员总数</span><strong>{{ students.length }}</strong><small>可分配训练对�?/small></div>
        <div class="stat-card accent"><span>训练项目</span><strong>{{ companyTasks.length }}</strong><small>本公司可�?/small></div>
        <div class="stat-card"><span>分配记录</span><strong>{{ assignments.length }}</strong><small>累计派发任务</small></div>
        <div class="stat-card success"><span>完成�?/span><strong>{{ completionRate }}%</strong><small>已完�?/ 已分�?/small></div>
      </section>
      ...
      <section v-else-if="currentMenu === 'task'" class="workspace-panel">
        <div class="panel-header">
          <div>
            <h2>本公司训练项�?/h2>
            <p>当前只开�?lead-train1 �?train2，可从总库添加到本公司�?/p>
          </div>
      ...
      <section v-else-if="currentMenu === 'student'" class="workspace-panel">
        ...
        <div class="table-wrap">
          <table>
            <thead><tr><th>学员</th><th>已完�?/th><th>完成�?/th><th>近期任务</th></tr></thead>
            <tbody>
      ...
      <section v-else-if="currentMenu === 'analytics'" class="workspace-panel">
        ...
        <section class="analysis-grid">
          ...
          <div><span>平均�?/span><strong>{{ analytics.summary.average_score }}</strong></div>
          ...
          <article v-for="item in analytics.attempts" :key="item.attempt_id" class="attempt-card">
            <button class="attempt-main" @click="toggleAttempt(item)">
              <div>
                ...
                <span>#{{ item.attempt_id }} · {{ item.scene_name }} · {{ item.completed_sub_count || 0 }}/{{ item.total_sub_count || item.sub_results?.length || 0 }} 子项目完�?/span>
              </div>
              ...
              <section v-for="sub in item.sub_results" :key="sub.sub_task_id" class="sub-card" :class="{ pending: sub.status !== 'done' }">
                <button class="sub-main" @click="toggleSub(item, sub)">
                  <div>
                    ...
                    <span>{{ sub.status === "done" ? "已完�? : "未完�? }} · 错误 {{ sub.error_count || 0 }} / 安全 {{ sub.safety_error_count || 0 }}</span>
                  </div>
                  <div class="sub-score">
                ...
                <div v-if="isSubExpanded(item, sub)" class="detail-box">
                  ...
                  <div class="step-grid">
                    <div v-for="step in sub.steps" :key="`${item.attempt_id}-${sub.sub_task_id}-step-${step.index}`" class="step-row">
                      <strong>{{ Number(step.index) + 1 }}. {{ step.name || step.stepName || "未命名步�? }}</strong>
                      <span>{{ step.expectedAction || step.expected_action || "未记录期望动�? }}</span>
                      <small>{{ step.completed ? "已完�? : "未完�? }} · 错误 {{ step.mistakeCount || step.mistake_count || 0 }}</small>
                    </div>
                  </div>
        ...
        <div class="dialog-table-wrap">
          <table>
            ...
            <tbody>
              ...
              <tr v-if="!availableGlobalTasks.length"><td colspan="4" class="empty-cell">总库中暂无更多可用项�?/td></tr>
            </tbody>
          </table>
        </div>
        <div class="dialog-actions">
          <span class="pick-count">已�?{{ selectedGlobalTasks.length }} �?/span>
          <div>
...
const username = localStorage.getItem("username") || "管理�?;
...
const menus = [
  { key: "assign", title: "训练分配", desc: "学员训练派发", mark: "�? },
  { key: "task", title: "训练项目", desc: "本公司可用训�?, mark: "�? },
  { key: "student", title: "学员总览", desc: "状态与进度", mark: "�? },
  { key: "analytics", title: "训练分析", desc: "成绩与错�?, mark: "�? }
];
...
const completionRate = computed(() => {
  ...
  const done = assignments.value.filter(a => a.status === "done" || a.status === "已完�?).length;
...
function studentCompleted(sid) { return assignments.value.filter(a => a.user_id === sid && (a.status === "done" || a.status === "已完�?)).length; }
...
function severityText(severity) { return severity === "safety" ? "安全" : severity === "warning" ? "警告" : "普�?; }
...
async function openTaskLibrary() {
  ...
  try {
    const r = await axios.get("http://60.205.176.200:8000/task/global/list");
...
async function addSelectedTasks() {
  ...
  for (const tid of selectedGlobalTasks.value) {
    try {
      await axios.post(`http://60.205.176.200:8000/task/company/${adminCompanyId}/add`, { task_id: tid });
      ok++;
    } catch (e) {}
  }
...
async function loadDashboard() {
  ...
  try {
    ...
    if (adminCompanyId) {
      const r = await axios.get(`http://60.205.176.200:8000/task/company/${adminCompanyId}`);
