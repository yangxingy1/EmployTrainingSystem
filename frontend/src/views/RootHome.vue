...
async function loadAdmins() {
  try { const r = await axios.get(api("/root/admins")); admins.value = r.data || []; } catch (e) {}
}
async function loadStatistics() {
  try {
    const r = await axios.get(api("/root/statistics"));
...
async function loadAllTasks() {
  try { const r = await axios.get(api("/task/list")); allTasks.value = r.data || []; } catch (e) {}
}
async function loadCompanyTasks() {
  try { const r = await axios.get(api("/root/company-tasks")); companyTasks.value = r.data || []; } catch (e) {}
}

// ============ 公司操作 ============
async function createCompany() {
  try { await axios.post(api("/root/companies"), newCompany.value); showCreate.value = false; newCompany.value = { name: "", code: "" }; await loadCompanies(); await loadStatistics(); } catch (e) { alert(e.response?.data?.detail || "创建失败"); }
}
async function changeStatus(item) {
  try { await axios.patch(api(`/root/companies/${item.id}`), { status: item.status === "active" ? "inactive" : "active" }); await loadCompanies(); } catch (e) { alert(e.response?.data?.detail || "操作失败"); }
}
async function deleteCompany(item) {
  if (!confirm(`确定删除公司"${item.name}"？`)) return;
  try { await axios.delete(api(`/root/companies/${item.id}`)); await loadCompanies(); await loadAdmins(); await loadStatistics(); await loadCompanyTasks(); } catch (e) { alert(e.response?.data?.detail || "删除失败"); }
}

// ============ 管理员操作 ============
async function createAdmin() {
  try { await axios.post(api("/root/admins"), newAdmin.value); showAdminCreate.value = false; newAdmin.value = { username: "", password: "", company_id: null }; await loadAdmins(); await loadStatistics(); } catch (e) { alert(e.response?.data?.detail || "创建失败"); }
}
async function deleteAdmin(admin) {
  if (!confirm(`确定删除管理员"${admin.username}"吗？`)) return;
  try { await axios.delete(api(`/root/admins/${admin.id}`)); await loadAdmins(); await loadStatistics(); } catch (e) { alert(e.response?.data?.detail || "删除失败"); }
}
...
async function saveTask() {
  ...
  try {
    if (editingTask.value) {
      await axios.put(api(`/task/${editingTask.value.id}`), { title: taskForm.title, description: taskForm.description });
    } else {
      await axios.post(api("/task/create"), { title: taskForm.title, description: taskForm.description });
    }
    closeTaskDialog();
    await loadAllTasks();
...
async function deleteTaskItem(task) {
  ...
  try { await axios.delete(api(`/task/${task.id}`)); await loadAllTasks(); await loadCompanyTasks(); } catch (e) { alert(e.response?.data?.detail || "删除失败"); }
}
...
async function doAssignCompany() {
  ...
  try {
    await axios.post(api("/root/company-tasks"), { company_id: assignCompanyId.value, task_id: assigningTask.value.id });
...
async function removeCompanyTask(ct) {
  ...
  try { await axios.delete(api(`/root/company-tasks/${ct.id}`)); await loadCompanyTasks(); } catch (e) { alert(e.response?.data?.detail || "操作失败"); }
}
